...
src_store = zarr.NestedDirectoryStore('path_to_parent_array_group')
src_root = zarr.open_group(store=src_store, mode = 'r')
src_arr = src_root['name_of_the_array']
out_chunks = (212,) * 3


dest_store = zarr.NestedDirectoryStore('path_to_array_group')
...
def create_multiscale(src_arr: zarr.Array, dest_root: zarr.Group, out_chunks: Tuple[int, ...], client: Client, num_workers: int, comp: Codec):
    ...
    for level in range(1, num_levels - 1):
        ...
        source_arr = dest_root[f's{level}']
...
if __name__ == '__main__':
    ...
    create_multiscale(src_arr=src_arr, dest_root=dest_root, out_chunks=out_chunks, client=client, num_workers=1200, comp=Zstd(level=6))
